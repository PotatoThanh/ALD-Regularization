...
def ald_trades_loss(model,
    ...
    x_adv = x_particle.detach() + 0.001 * torch.randn(x_particle.shape).cuda().detach()
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss_kl = criterion_kl(F.log_softmax(model(x_adv.reshape(batch_size*n, c, w, h)), dim=1),
                                       F.softmax(model(x_particle), dim=1))
            # print(loss_kl.sum().item())
            score_func = torch.autograd.grad(loss_kl.sum(), [x_adv])[0]
            ...
            # x_adv = x_adv.detach() + 1000*step_size * phi.detach()
    ...
    loss_robust = (1.0 / (batch_size*n)) * criterion_kl(F.log_softmax(model(x_adv), dim=1),
                                                    F.softmax(model(x_particle), dim=1))
